...
function [  nReset, tWindow, PSTH, ...
    ...
    psthPupilReset_gather_resetTest( pupilResetIdx, pupil, dpupil, intv, tmargin, pthresh, session)

fPupilResetIdx = find(pupilResetIdx);
fprintf(1,'#pupil resets: %d\n', length(fPupilResetIdx) )
...
% for n=1:session.nRois
%     ypeak = max(ymean0margin(n,:),[],2);
%     ymean0marginPass  =  ypeak < meanDeconvPupil(n,:);
%     pmean0margin(n,1) = mean( ymean0marginPass ) ;
% end
...
DeconvMeanShuffle = zeros( 1, tWindow, session.nRois, nIter );
fprintf(1,'Shuffling..\n')
for iter = 1:nIter
   ...
   Dtmp = nan( nReset, tWindow, session.nRois );
   for iR = 1:nReset
       Dtmp(iR,:,:) = circshift( DeconvPupilMat(iR,:,:) , [0 shif(iR) 0]);
   end
   DeconvMeanShuffle(1,:,:, iter) = nanmean(Dtmp,1); 

%   for iR = 1:nReset
%       x = squeeze(DeconvPupilMat(iR,:,:)).';
%       Dtmp(iR,:,:) = circshift( x , [0 shif(iR)]);
%   end
...
DeconvMeanData = (nanmean(DeconvPupilMat,1)); 
...
idxWindow = intv+[-margin:margin];
clear scoreData scoreShuffle scorePercentile scoreQuantile quan95

scoreData = zeros( session.nRois, tWindow );
pctData   = zeros( session.nRois, tWindow );
scoreShuffle = zeros( session.nRois, nIter );
for n=1:session.nRois
    
    %scoreData(n,1)      = max( DeconvMeanData(n,idx),[],2 )  ;
    %scoreShuffle(n,:)   = squeeze( max( DeconvMeanShuffle(n,idx,:),[],2) );

    scoreData(n,:)      = DeconvMeanData(1,:,n) ;
    scoreShuffle(n,:)   = squeeze( max( DeconvMeanShuffle(1,:,n,:),[],2) );
    for k = 1:tWindow
        scorePercentileMat(n,k) = mean( scoreData(n,k) > scoreShuffle(n,:)) ;
    end
    scorePercentile(n,1) = max(scorePercentileMat(n,idxWindow),[],2);
    ...
    if debugMode
        ...
        plot( t, scoreData(n,:))
        hold all
        plot( t, repmat( quan95(n), 1, tWindow ) )
        title( sprintf('pctData : %g', max(pctData(n,:)) ))
        pause
    end
...
%    pupilCellShuffle(n,1) = any( scorePercentile(n,:) > qthresh );
    pupilCellShuffle(n,1) = any( scorePercentileMat(n,idxWindow) > qthresh );
...
statsShuffle = v2struct( scoreData, scoreShuffle, scorePercentile, qthresh, pupilCellShuffle, quantile95,quantile99);
