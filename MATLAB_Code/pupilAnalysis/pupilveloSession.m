function Results = pupilveloSession(session,in_light,walls_only,...
    selectTrialFactor,tag, plotfig1,plotfig2,plotfig3,plotfig4, ...
    rootdir, timeCut, showPlots, plotfig0)
...
pupilResetIdx = markPupilReset(session,'threshStdDpupil',threshStdDPupil,...
     ...
     'showPlots',showPlots,'deltaT',1,'maxFrequency',10,'threshStdDPupil', threshStdDPupil,...
    'useFindpeaks', true);
...
pupilData = v2struct(pupil1,dpupil1,pupil2,dpupil2,quantileReset,pupilResetIdx,quantileReset,threshStdDPupil, freqPupilReset);
 
% Figure over time
manudir = localpath('manudir');
savedir = sprintf('%s/figures/figureS8-pupil1/%s-examples',manudir,datestr(now,'yyyymmdd'));
...
pupilVeloSession_plots(Results,session,pupilData,...
    ...
    'plotfig1', plotfig1, 'plotfig2',plotfig2, 'plotfig3', plotfig3, 'savedir',rootdir,...
    'tag',tag);
